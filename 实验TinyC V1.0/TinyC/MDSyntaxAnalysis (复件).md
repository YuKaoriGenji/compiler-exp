...
static EXPVAL Prod_B1(EXPVAL val1)
{
	...
	if (lookahead.token==SYN_OR)
	{
        ...
        if(val1.type==ID_CHAR){val1.val.intval=bval1;}
        else if(val1.type==ID_INT) {val1.val.charval=bval1;}
		val=Prod_B1(val1);
		return(val);
	}
	else
...
static EXPVAL Prod_F()
{
	...
	else if (lookahead.token==SYN_PAREN_L)
	{
		...
		match(SYN_PAREN_L);
		val=Prod_E();
        printf("E.type=%d,E.val=%d \n",val.type,val.val.intval);
		match(SYN_PAREN_R);
	}
    else if (lookahead.token=SYN_CHAR)
    {
